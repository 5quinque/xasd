def test_read_playlists_me(create_playlist, client):
    ...
    with client as c:
        response = c.get("/playlist/me", headers={"Authorization": f"Bearer {token}"})
...
def test_create_playlist_no_token(client):
    with client as c:
        response = c.post("/playlist", json={"name": "playlist_name"})
    assert response.status_code == 401
    assert response.json() == {"detail": "Not authenticated"}
...
def test_add_track_to_playlist_no_token(env, client):
    with client as c:
        response = c.patch("/playlist/1/track/1")
    assert response.status_code == 401
    assert response.json() == {"detail": "Not authenticated"}
...
def test_remove_track_from_playlist_no_token(env, client):
    with client as c:
        response = c.delete("/playlist/1/track/1")
    assert response.status_code == 401
    assert response.json() == {"detail": "Not authenticated"}
...
def test_delete_playlist(create_playlist, client):
    token = create_playlist["token"]
    with client as c:
        response = c.delete("/playlist/1", headers={"Authorization": f"Bearer {token}"})
